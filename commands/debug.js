...
module.exports.run = async (client, message, args, prefix, player, db) => {
    ...
    if (!args[0]) return message.channel.send(`Please type \`${prefix}help debug\` to learn how to use this command.`)
    ...
    if (args[1]) {
        switch (args[1].toLowerCase()) {
            case "image":
                ...
                if (!test) return message.channel.send(`That Bey's image cannot be updated.`);
                ...
                message.channel.send("Image updated!");
                break;
            default:
                message.channel.send("No debug option found.")
        }
        return;
    }

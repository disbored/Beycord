...
module.exports.run = async (client, message, args, prefix, player, db) => {
  ...
  let user;
  if (message.mentions[0]) user = await message.guild.member.cache.get(message.mentions[0].id);
  else user = await message.guild.member.cache.get(args[0]);
  if (!user) return message.reply("no user found.");
  if (user.id === message.author.id) return message.channel.send("You paid yourself all the Valtz that you have. You are now officially broke while yourself is now officially rich thanks to you.");
  ...
  message.channel.send(`✅ <:valtz:999800344848650410>${amount} successfully paid to ${user.effectiveName}.`);
  let webhookembed2 = new Discord.EmbedBuilder()

...
module.exports.run = async (client, message, args, prefix, player, db) => {
    ...
    switch (args[0]) {
        case "equip":
            if (stats.launcher !== "default") return message.channel.send(`Please unequip your current launcher before continuing. You can do \`${prefix}lc unequip\` to unequip your current launcher.`)
            if (!args[1]) return message.channel.send(`Please type \`${prefix}help launcher\` to know how to use this command properly.`);
            let index2 = parseInt(args[1]) - 1;
            let launcher2 = stats.items[index2];
            if (!launcher2 || !launcher2.name.toLowerCase().includes("launcher")) return message.channel.send("No launcher found.");
            stats.items.splice(index2, 1);
            db.collection("users").updateOne({ _id: message.author.id }, { $set: { launcher: launcher2, items: stats.items } });
            message.channel.send(`Equipped ${launcher2.name} ${launcher2.var}!`);
            break;
        case "unequip":
            if (stats.launcher === "default" || !stats.launcher) return message.channel.send(`You don't have any launcher equipped. You can do \`${prefix}lc equip\` to equip a launcher.`)
            ...
            message.channel.send(`Unequipped ${stats.launcher.name} ${stats.launcher.var}!`);
            break;
        default:
